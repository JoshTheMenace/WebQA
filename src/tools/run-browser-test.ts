...
import { TestDefinition, TestStep, TestResult } from '../types/dsl.js';
import path from 'path';

// Validate that a step has the required properties for its action type
function validateStep(step: unknown, index: number): TestStep {
...
function validateTestDefinition(input: unknown): TestDefinition {
  ...
  const validatedSteps: TestStep[] = obj.steps.map((step, index) =>
